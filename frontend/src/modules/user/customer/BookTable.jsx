...
const BookTable = () => {
  ...
  const getAllTables = async () => {
    try {
      const res = await axios.get('http://localhost:8001/api/user/getalltables');
  ...
  const bookingTable = async (hotelId, tabletype) => {
    try {
      const res = await axios.post(`http://localhost:8001/api/user/booktable/${hotelId}`, {...bookingDetails, tabletype}, {
