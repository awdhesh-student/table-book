...
const AllBookings = () => {
  ...
  const getAllBookings = async () => {
    try {
      const res = await axios.get('http://localhost:8001/api/user/allbooking', {
  ...
  const handleApprove = async (bookingId, status) => {
    try {
      const res = await axios.post('http://localhost:8001/api/user/approve', { bookingId, status }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        }
      })
  ...
  const handleReject = async (bookingId, status) => {
    try {
      const res = await axios.post('http://localhost:8001/api/user/reject', { bookingId, status }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        }
      })
