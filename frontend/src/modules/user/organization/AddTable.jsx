...
const AddTable = ({userId}) => {
   ...
   const handleChange = (e) => {
      ...
      if (name.startsWith('img')) {
         const index = parseInt(name.replace('img', ''), 10); // Extract the index from the 'imgX' name
         const updatedImages = [...hoteldata.img_url]; // Create a copy of the current images array
         updatedImages[index - 1] = value; // Update the URL at the appropriate index
   ...
   const handleSubmit = async (e) => {
      ...
      try {
         const response = await axios.post(`http://localhost:8001/api/user/addtable/${userId}`, data, {
      ...
      <div className='d-flex justify-content-center align-item-center'>
         <form onSubmit={handleSubmit} className='w-50 text-center border border-3 rounded-3 p-3 m-3'>
            ...
            <div className="form-outline mb-4">
               <label className="form-label" for="address">Hotel Address</label>
            ...
            <div className="form-outline mb-4">
               <label className="form-label" for="phone">Hotel Phone</label>
               <input name='phone' onChange={handleChange} value={hoteldata.phone} type="number" id="phone" className="form-control border border-2" />
            </div>

            <div className="form-outline mb-4">
               <label className="form-label" for="img1">Hotel/Table Image</label>
               <input name='img1' onChange={handleChange} value={hoteldata.img1} placeholder="img1 url" type="text" className="form-control border border-2" />
               <input name='img2' onChange={handleChange} value={hoteldata.img2} placeholder="img2 url" type="text" className="form-control border border-2 my-2" />
            </div>
            ...
            <div className="form-outline mb-4">
               <label className="form-label" for="price">Table Price(per person)</label>
